(function (global, factory) {
  ...
  function handleForDirective(
    ...
    items.forEach((item, index) => {
      ...
      if (!nextEl || nextEl.__x_for_key === undefined) {
        ...
        nextEl = lookAheadForMatchingKeyedElementAndMoveItIfFound(
          nextEl,
          currentKey
  ...
  function generateKeyForIteration(
    ...
    let bindKeyAttribute = getXAttrs(el, "bind").filter(
      (attr) => attr.value === "key"
  ...
  function warnIfNotTemplateTag(el) {
    if (el.tagName.toLowerCase() !== "template")
      console.warn(
        "Alpine: [x-for] directive should only be added to <template> tags."
      );
  }
  ...
  function registerListener(
    ...
    let eventKey = "__x_" + event + "_handler";
    ...
    if (modifiers.includes("away")) {
      ...
      if (el[eventKey]) {
        document.removeEventListener(event, el[eventKey]);
      }
      el[eventKey] = handler;
      ...
      if (listenerTarget[eventKey]) {
        listenerTarget.removeEventListener(event, listenerTarget[eventKey]);
      }
      listenerTarget[eventKey] = handler;
  ...
  class Component {
    constructor(el, seedDataForCloning = null, reInitialize = false) {
      this.$el = el;
      ...
      this.unobservedData = seedDataForCloning
        ? seedDataForCloning
        : saferEval(dataExpression, {});
